...
- Owner coverage in the property-risk table: 88.5% (9292 of 10500 properties).

## Modeling Method
- Target: `will_receive_high_risk_violation_next_period`, defined as at least one new `high risk (proxy)` violation within 730 days after the cutoff date 2024-04-28.
...
- `history_high_risk_share` (0.707)
- `violations_per_year` (0.373)
- `days_since_last_violation` (0.373)

Top negative coefficient directions:
- `open_violations` (-0.350)
- `history_open_share` (-0.240)
- `total_violations` (-0.091)
...
- Matched ZIP summary: 20 ZIP codes with student metric `all_students`; correlation with total violations = -0.4950, correlation between `Students per Property` and violation intensity = -0.2554.

## Preliminary Results and Interpretation
- Modeling frame: 9828 properties, 68 positive examples (0.0069 positive rate).
- Holdout metrics: accuracy=0.8932, balanced_accuracy=0.5820, precision=0.0410, recall=0.2609, f1=0.0708, roc_auc=0.7129.
- Majority-class accuracy is 0.9844, so balanced accuracy and recall are more informative than raw accuracy because the target event is rare.
